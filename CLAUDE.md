...
```
C:\Users\ignot\OneDrive\Desktop\projects\projects\church-website\
```
...
4. **Every file under** `C:\Users\ignot\OneDrive\Desktop\projects\projects\church-website\` — the user may have edited or added housekeeping between sessions. **Do not assume any prior state.** Read them all, fresh, every session. This is non-negotiable.
...
If the vault folder `C:\Users\ignot\OneDrive\Desktop\projects\projects\church-website\` does not exist yet, this is a brand-new project and you need to set it up before doing any code work:
...
4. **Bake the project name into this `CLAUDE.md`** by replacing every `<this-project-folder>` and `church-website` placeholder above with the actual name. Save.
...
3. Update affected vault housekeeping (which files to update is in `workflow.md`).
